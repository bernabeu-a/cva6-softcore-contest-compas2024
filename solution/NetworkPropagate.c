                  ...
                  int nb_iterations)
{
    ...
    */
    ...
    for (int iter = 0; iter < nb_iterations; ++iter) {
        ...
        input_ = (inputs[2*iter]) | (inputs[2*iter+1]<<8) | (inputs[2*iter+2]<<16) | (inputs[2*iter+3]<<24);
        weights_ = (weights[2*iter]) | (weights[2*iter+1]<<8) | (weights[2*iter+2]<<16) | (weights[2*iter+3]<<24);
        asm volatile
        (
        "custom1   %[z], %[x], %[y]\n\t"
        : [z] "=r" (res)
                  ...
                  int nb_iterations)
{
    ...
    */
    ...
    for (int iter = 0; iter < nb_iterations/2; ++iter) {
        ...
        int32_t w1 = (weights[8*iter]) | (weights[8*iter+1]<<8) | (weights[8*iter+2]<<16) | (weights[8*iter+3]<<24);
        int32_t w2 = (weights[8*iter+4]) | (weights[8*iter+5]<<8) | (weights[8*iter+6]<<16) | (weights[8*iter+7]<<24);
        uint32_t i1 = (inputs[8*iter]) | (inputs[8*iter+1]<<8) | (inputs[8*iter+2]<<16) | (inputs[8*iter+3]<<24);
        uint32_t i2 = (inputs[8*iter+4]) | (inputs[8*iter+5]<<8) | (inputs[8*iter+6]<<16) | (inputs[8*iter+7]<<24);
...
static void convcellPropagate1Custom(
    const UDATA_T* __restrict inputs,
    UDATA_T* __restrict outputs,
    const BDATA_T* __restrict biasses,
    const WDATA_T* __restrict weights,
...
static void convcellPropagate2Custom(
    const UDATA_T* __restrict inputs,
    UDATA_T* __restrict outputs,
    const BDATA_T* __restrict biasses,
    const WDATA_T* __restrict weights,
...
static void fccellPropagateCustomUDATA_T(
    const UDATA_T* __restrict inputs,
    UDATA_T* __restrict outputs,
    const BDATA_T* __restrict biasses,
    const WDATA_T* __restrict weights,
...
void propagate(const UDATA_T* inputs, Target_T* outputs, UDATA_T* maxPropagate_val)
{
    ...
    convcellPropagate1Custom(inputs , conv1_output, conv1_biases, conv1_weights, 8,
    ...
    convcellPropagate2Custom(conv1_output , conv2_output, conv2_biases, conv2_weights, 8,
    ...
    fccellPropagateCustomUDATA_T(conv2_output , fc1_output, fc1_biases, fc1_weights, 8,
